	...
	chrome.runtime.onMessage.addListener(
		function(request, sender, sendResponse) {
			...
			if (request.greeting == "please open the tray"){
				...
				if ($('#he_tray').length == 0){
					// Get/Set datetime
					chrome.storage.local.get(['rate_ui_last_opened', 'purple_upsell_shown'], function(result){
						
						// Get and reset last opened date
						var rate_ui_last_opened = result.rate_ui_last_opened;
						chrome.storage.local.set({'rate_ui_last_opened': Date.now() });//Date.now() });
						...
						// Inject tray html template into body 
						...
						// Callout creation
						$("body").append("<div id='he_callout' class='he_reset' style='display:none;'><div id='he_heuristic_label'>Add a heuristic</div><select id='he_heuristic_select' ></select><div id='he_notes_label'>Notes</div><textarea id='he_notes' class='he_callout_textarea he_reset'></textarea><div id='he_recommendation_label'>Recommendation</div><textarea id='he_recommendation' class='he_callout_textarea he_reset'></textarea>"+
						"Rate the Criteria:<div id='criteria_container'><div id='he_intuitively_container'><span id='he_intuitively_label' class='he_reset'>Intuitively</span><select id='he_intuitively_dropdown'><option>0</option><option>1</option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option></select></div>"+
						"<div id='he_comfortable_container'><span id='he_comfortable_label' class='he_reset'>Comfortable</span><select id='he_comfortable_dropdown'><option>0</option><option>1</option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option></select></div>"+
						"<div id='he_reduceCognitiveLoad_container'><span id='he_reduceCognitiveLoad_label' class='he_reset'>Reduce cognitive load</span><select id='he_reduceCognitiveLoad_dropdown'><option>0</option><option>1</option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option></select></div>"+
						"<div id='he_consistent_container'><span id='he_consistent_label' class='he_reset'>Consistent</span><select id='he_consistent_dropdown'><option>0</option><option>1</option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option></select></div></div>"+
						"<button class='he_callout_button' id='he_callout_cancel'>Cancel</button><button id='he_callout_save' class='he_callout_button'>Save</button></div>");
								...
								setTimeout(function(){
									chrome.runtime.sendMessage({greeting: "take_screenshot"}, function(response) {
										...
										imageObj.onload = function() {
											...
											var save_intuitively = $('#he_intuitively_dropdown').val();
											var save_comfortable = $('#he_comfortable_dropdown').val();
											var save_reduceCognitiveLoad = $('#he_reduceCognitiveLoad_dropdown').val();
											var save_consistent = $('#he_consistent_dropdown').val();
											var save_rate_all = (Number(save_intuitively) + Number( save_comfortable) + Number(save_reduceCognitiveLoad) + Number(save_consistent))/4
											...
											chrome.storage.local.get('saved_data', function(result){ 
												...
												$.ajax({
													...
													data: {
														...
														webpage_url1: webpage_url.href,
														save_heuristic1: save_heuristic,
														save_rate_all1: save_rate_all.toString(),
														save_notes1: save_notes,
														save_recommendation1: save_recommendation,
														save_screenshot1: save_screenshot
													},
									...
									function generateTable(saved_data){						
										var insert_table = "<div id='he_progress_close' class='he_close_button'>"+
															...
															"<table id='progress_table'>"+
																"<thead>"+
																	"<tr>"+
																		"<td>Screenshot</td><td>Heuristic</td><td>Total Rate</td><td>Notes</td><td>Recommendation</td>"+
																	"</tr>"+
																"</thead>";
										for (var i = 0; i < saved_data.length; i++){
											...
											insert_table = insert_table + "<td>" + saved_data[i]['rate'] + "</td>";
									...
									chrome.storage.local.get('saved_data', function(result){ 
										...
										// Generate xml
											...
											for (i = 0; i < num_of_heuristics; i++){
												...
												if (saved_data[i].rate.length > 0){
													docx_html+= '<h2 class="rate">Total Rate of Criteria: '+ saved_data[i].rate + '</h2>';
												}
...
function close_callout(){
	$('#he_callout').css({'left':'0', 'top':'0', 'display':'none'});
	window.hover_enabled = true;
	$('#he_intuitively_dropdown').val("0");
	$('#he_comfortable_dropdown').val("0");
	$('#he_reduceCognitiveLoad_dropdown').val("0");
	$('#he_consistent_dropdown').val("0");
